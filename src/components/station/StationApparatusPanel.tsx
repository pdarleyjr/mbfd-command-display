...
export function StationApparatusPanel({ apparatus, className }: Props) {
  ...
  return (
    <GlassPanel
      ...
      right={<span className="text-[11px] uppercase tracking-wider text-faint"><span className="tnum">{list.length}</span> units</span>}
    >
      {list.length === 0 ? (
        <EmptyState icon={<Truck size={22} />} title="No apparatus assigned" />
      ) : (
